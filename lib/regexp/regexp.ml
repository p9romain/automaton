      ...
      | Union l ->
        ...
        (
          ...
          | _ -> (
            ...
            unique_l
            in(* Returns the (prefixe of size len, what it is after) *)
                ...
                if len = 0 then
                  ...
                  | _ ->
                    if len = 1 then
                      ...
                      None
              in
              match loop r len with
              | None -> None
              | Some (r, next) -> Some (simp @@ flatten r, simp @@ flatten next)
            in
            (* Calculates all the prefixe of size len for an union 
               Union of regexes is commutative, so it's not a problem
                if it's reversed
            *)
